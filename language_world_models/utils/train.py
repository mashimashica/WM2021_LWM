...
pred_z_seq      : (B_SIZE, pred_z_steps, z_dim)
"""
def train_lbf_batch(model_lbf, optimizer_lbf, \
    ...
    beta, beta_mean, beta_logstd, pred_z_seq  = \
            model_lbf(input_m, input_z)

    loss, kl_loss, pred_loss = \
            model_lbf.loss(target_z_seq, pred_z_seq, beta_mean, beta_logstd)
    loss.backward()
...
"""
def train_lbf_rnn_batch(model_lbf, optimizer_lbf, \
                    input_m_seqs, input_z_seqs, target_z_seqs):
    model_lbf.train()
    model_lbf.zero_grad()
     
    beta, beta_mean, beta_logstd, pred_z_seqs = \
...
# REINFORCE の更新
def train_reinforce_batch(model_reinforce, optimizer_reinforce, acts, rews, z, beta):
    model_reinforce.train()
    model_reinforce.zero_grad()

    loss, loss_a, loss_n_ent_a, loss_v  = model_reinforce.loss(acts, rews, z, beta)
    loss.backward()
    optimizer_reinforce.step()
    
    return loss.item(), loss_a.item(), loss_n_ent_a.item(), loss_v.item()
